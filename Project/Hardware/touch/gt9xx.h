...
/*
 * 触摸坐标必须与 NT35510 的 `LCD_X_LENGTH`/`LCD_Y_LENGTH` 全局变量一致。
 * 请在包含本头文件前先包含 `bsp_nt35510_lcd.h`（main.c / gt9xx.c 已如此）。
 */

/*flags??????????????????????????*/
//#define I2C_M_TEN		0x0010	/* ????????10????? */
...
/* ?????? printf??????? LCD/????????? */
#define GTP_INFO(fmt,arg...)           ((void)0)
#define GTP_ERROR(fmt,arg...)          ((void)0)
#define GTP_DEBUG(fmt,arg...)          ((void)0)
#define GTP_DEBUG_ARRAY(array, num)    ((void)0)
#define GTP_DEBUG_FUNC()               ((void)0)

#define GTP_SWAP(x, y)                 do { \
		uint32_t _swp_tmp = (uint32_t)(x); \
		(x) = (y); \
		(y) = _swp_tmp; \
	} while (0)
