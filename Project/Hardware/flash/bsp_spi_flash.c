...
/*
 * 【初学者导读】SPI NOR：
 * - Mode3（CPOL=高，CPHA=第 2 边沿采样）配合板载 Winbond/MX 兼容系列。
 * - 典型流程：WriteEnable → PageProgram/SectorErase → WaitForWriteEnd（轮询 WIP 位）。
 * - CS 低有效：`SPI_FLASH_CS_HIGH` 空闲时必须释放总线，避免多器件冲突。
 */
