...
/*
 * 【初学者导读 — ETH DMA 描述符】
 * - Rx/Tx 描述符里的 OWN 位区分「CPU 可读写的缓冲区」与「DMA 正在搬运的缓冲区」；
 *   lwIP 端口层在拷贝完帧后必须把 OWN 置回 DMA，否则会触发 RBUS/TUS 一类停滞。
 * - `DMATxDescToSet` / `DMARxDescToGet`：追踪环形链表当前提交位置；具体链表初始化见 ETH_DMARxDescChainInit。
 */

/* 工程中若已通过其它头定义 __EXRAM（如 section），则沿用；否则为空，避免 ARMCC #65 */
#ifndef __EXRAM
#define __EXRAM
#endif
